  ...
  <div>
    ...
    <div v-else class="grid gap-6 lg:grid-cols-[1fr_320px]">
            ...
            <div v-for="i in cart.drawItems" :key="i.key" class="flex items-center gap-4 rounded-2xl border-2 border-accent-500/30 bg-accent-500/5 p-3">
              ...
              <div class="flex-1">
                ...
                <p class="text-xs text-slate-500">
                  1% advance on {{ money(i.listed_price) }} · one seat · pay the remaining 99% later, or keep the credit
                </p>
              </div>
      ...
      <div class="card h-fit p-5">
        ...
        <div v-if="cart.buyItems.length" class="mt-1 flex justify-between text-sm">
          <span class="text-slate-500">Wallet can cover up to</span><span class="text-brand-700">{{ money(cart.walletCap) }}</span>
        </div>
